...
if __name__ == "__main__":
    ...
    for file in files:
        ...
        contours = measure.find_contours(np.where(mask_np == guard_value, 0, mask_np), level=61)
        ...
        for contour in contours:
            ...
            # Visualise on image
            if(GCW_2>100):
              draw.text((max_y - 50,min_x), "Class: " + str(1), fill=(255, 0, 0), font=font)
              count1 += 1
              pollen_class = 1
            else:
              draw.text((max_y - 50,min_x), "Class: " + str(2), fill=(255, 0, 0), font=font)
              pollen_class = 2
              count2 += 1
            excel_output.append("," + str(id) + ", " + str(GCW_2) + ", " + str(GCW_1) + ", " +  str(pollen_class))
            draw.text((max_y - 50,min_x - 30), "ID: " + str(id), fill=(255, 0, 0), font=font)
            draw.text((max_y - 50,min_x - 10), "Width: " + "{:.3f}".format(GCW_1), fill=(0, 0, 0), font=font)
            draw.text((max_y - 50,min_x - 40), "Length: " + "{:.3f}".format(GCW_2), fill=(0, 0, 0), font=font)
